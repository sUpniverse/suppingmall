...
CREATE TABLE IF NOT EXISTS `suppingmall`.`user` (
  ...
  `store_registered_date` TIMESTAMP(3) NULL DEFAULT NULL,
  `store_updated_date` TIMESTAMP(3) NULL DEFAULT NULL,
  PRIMARY KEY (`user_id`),
...
CREATE TABLE IF NOT EXISTS `suppingmall`.`board` (
  ...
  `delete_yes_or_no` CHAR(1) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `suppingmall`.`product` (
  ...
  `status` VARCHAR(4) NOT NULL COMMENT '물품 상태',
...
CREATE TABLE IF NOT EXISTS `suppingmall`.`delivery` (
  ...
  INDEX `fk_delivery_orderitem_id_idx` (`orderitem_id` ASC) VISIBLE,
  CONSTRAINT `fk_delivery_orderitem_id`
    FOREIGN KEY (`orderitem_id`)
    REFERENCES `suppingmall`.`orderitem` (`orderitem_id`))
ENGINE = InnoDB
AUTO_INCREMENT = 77
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `suppingmall`.`payment` (
  ...
  INDEX `fk_payment_orderitem_id_idx` (`orderitem_id` ASC) VISIBLE,
  CONSTRAINT `fk_payment_orderitem_id`
    FOREIGN KEY (`orderitem_id`)
    REFERENCES `suppingmall`.`orderitem` (`orderitem_id`))
ENGINE = InnoDB
AUTO_INCREMENT = 95
...
CREATE TABLE IF NOT EXISTS `suppingmall`.`orderItem` (
  ...
  `ordered_date` TIMESTAMP(3) NULL DEFAULT NULL,
  `updated_date` TIMESTAMP(3) NULL DEFAULT NULL,
  `status` CHAR(4) NULL DEFAULT NULL,
  `review_yes_or_no` CHAR(1) NULL DEFAULT NULL,
  `delivery_delivery_id` BIGINT(20) NOT NULL,
  ...
  INDEX `fk_order_item_buyer_id_idx` (`buyer_id` ASC) VISIBLE,
  INDEX `fk_order_item_seller_id_idx` (`seller_id` ASC) VISIBLE,
  INDEX `fk_orderItem_delivery1_idx` (`delivery_delivery_id` ASC) VISIBLE,
  INDEX `fk_orderItem_payment1_idx` (`payment_payment_id` ASC) VISIBLE,
  CONSTRAINT `fk_orderitem_buyer_id`
    FOREIGN KEY (`buyer_id`)
    REFERENCES `suppingmall`.`user` (`user_id`),
  CONSTRAINT `fk_orderitem_order_id`
    ...
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_orderitem_product_option_id`
    FOREIGN KEY (`product_id` , `product_option_id`)
    REFERENCES `suppingmall`.`product_option` (`product_id` , `product_option_id`),
  CONSTRAINT `fk_orderitem_seller_id`
    FOREIGN KEY (`seller_id`)
    REFERENCES `suppingmall`.`user` (`user_id`),
  CONSTRAINT `fk_orderItem_delivery1`
    FOREIGN KEY (`delivery_delivery_id`)
    REFERENCES `suppingmall`.`delivery` (`delivery_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_orderItem_payment1`
...
CREATE TABLE IF NOT EXISTS `suppingmall`.`product_detail` (
  ...
  `spec_1` VARCHAR(100) NULL DEFAULT NULL,
  `spec_2` VARCHAR(100) NULL DEFAULT NULL,
  `spec_3` VARCHAR(100) NULL DEFAULT NULL,
  `spec_4` VARCHAR(100) NULL DEFAULT NULL,
  `spec_5` VARCHAR(100) NULL DEFAULT NULL,
  `spec_6` VARCHAR(100) NULL DEFAULT NULL,
  `spec_7` VARCHAR(100) NULL DEFAULT NULL,
  `spec_8` VARCHAR(100) NULL DEFAULT NULL,
  `spec_9` VARCHAR(100) NULL DEFAULT NULL,
  `spec_10` VARCHAR(100) NULL DEFAULT NULL,
  `manuplated_date` DATETIME NULL DEFAULT NULL,
  `as_number` VARCHAR(45) NULL DEFAULT NULL,
  `contry_of_origin` VARCHAR(100) NULL DEFAULT NULL,
...
-- -----------------------------------------------------
-- Table `suppingmall`.`qna`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `suppingmall`.`qna` (
  ...
  `parent_id` BIGINT(20) NULL DEFAULT NULL,
  PRIMARY KEY (`qna_id`),
  INDEX `fk_qna_user_id_idx` (`user_id` ASC) VISIBLE,
  INDEX `fk_product_id_idx` (`product_id` ASC) VISIBLE,
  INDEX `fk_qna_parent_id_idx` (`parent_id` ASC) VISIBLE,
  CONSTRAINT `fk_qna_parent_id`
    FOREIGN KEY (`parent_id`)
    REFERENCES `suppingmall`.`qna` (`qna_id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_qna_product_id`
    FOREIGN KEY (`product_id`)
    REFERENCES `suppingmall`.`product` (`product_id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
...
-- -----------------------------------------------------
-- Table `suppingmall`.`review`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `suppingmall`.`review` (
  ...
  INDEX `fk_reivew_user_id_idx` (`user_id` ASC) VISIBLE,
  INDEX `fk_review_product_id_idx` (`product_id` ASC) VISIBLE,
  INDEX `fk_review_orderItem_id_idx` (`orderitem_id` ASC) VISIBLE,
  CONSTRAINT `fk_reivew_user_id`
    FOREIGN KEY (`user_id`)
    REFERENCES `suppingmall`.`user` (`user_id`),
  CONSTRAINT `fk_review_orderItem_id`
    FOREIGN KEY (`orderitem_id`)
    REFERENCES `suppingmall`.`orderitem` (`orderitem_id`),
